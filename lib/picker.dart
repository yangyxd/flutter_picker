...
class Picker {
    ...
    return Dialog.showDialog<List<int>>(
        ...
        builder: (BuildContext context) {
          ...
          final _confirm = PickerWidgetState._buildButton(
              context, confirmText, confirm, confirmTextStyle, false, theme, () async {
  ...
  static ButtonStyle _getButtonStyle(ButtonThemeData? theme, [isCancelButton = false]) =>
...
class _PickerWidget<T> extends StatefulWidget {
  ...
  final bool isModal;
  _PickerWidget({Key? key, required this.picker, this.themeData, required this.isModal})
      : super(key: key);
...
class PickerWidgetState<T> extends State<_PickerWidget> {
    ...
    if (!picker.hideHeader) {
      if (picker.builderHeader != null) {
        _body.add(picker.headerDecoration == null
            ...
            : DecoratedBox(
                child: picker.builderHeader!(context), decoration: picker.headerDecoration!));
      } else {
  ...
  List<Widget> _buildHeaderViews(BuildContext context) {
    ...
    final _cancel = _buildButton(context, picker.cancelText, picker.cancel, picker.cancelTextStyle,
        true, theme, () => picker.doCancel(context));
    if (_cancel != null) {
      items.add(_cancel);
    }
  ...
  static Widget? _buildButton(BuildContext context, String? text, Widget? widget,
      TextStyle? textStyle, bool isCancel, ThemeData? theme, VoidCallback? onPressed) {
    if (widget == null) {
      String? _txt = text ??
          (isCancel
            ...
            child: _wait
                ...
                : StatefulBuilder(
                    builder: (context, state) {
                      ...
                      final _view = _buildCupertinoPicker(
                          context, i, _length, adapter, _lastIsEmpty ? ValueKey(_length) : null);

                      if (_lastIsEmpty ||
                          (!picker.changeToFirst && picker.selecteds[i] >= _length)) {
  ...
  Widget _buildCupertinoPicker(
      BuildContext context, int i, int _length, PickerAdapter adapter, Key? key) {
...
class PickerDataAdapter<T> extends PickerAdapter<T> {
  ...
  Widget buildItem(BuildContext context, int index) {
    ...
    if (picker!.onBuilderItem != null) {
      final _v =
          picker!.onBuilderItem!(context, item.value.toString(), item.text, isSel, _col, index);
  ...
  int getLength() {
    ...
    switch (_type) {
      ...
      case 4: // minute
        ...
        break;
      case 7: // hour am/pm
        if ((minHour != null && minHour! >= 0) || (maxHour != null && maxHour! <= 23)) if (_colAP <
            0) {
    ...
    for (int i = 0; i < _maxLevel; i++) {
      ...
      switch (colType) {
        ...
        case 6:
          // am/pm
          picker!.selecteds[i] =
              (sh > 12 || (sh == 12 && (value!.minute > 0 || value!.second > 0))) ? 1 : 0;
          break;
        case 7:
